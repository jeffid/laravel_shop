    ...
    public function index(Request $request)
    {
        ...
        if ($request->input('category_id') && $category = Category::find($request->input('category_id'))) {
            if ($category->is_directory) {
                ...
                $params['body']['query']['bool']['filter'][] = ['prefix' => ['category_path' => $category->path . $category->id . '-']];
            } else {
...
//        dd($result);
        ...
        if (isset($result['aggregations'])) {
            ...
            $properties = collect($result['aggregations']['properties_1']['properties_name']['buckets'])
                ->map(function ($bucket) {
                    // 通过 map 方法取出我们需要的字段
                    return [
                        'key' => $bucket['key'],
        ...
        return view('products.index', [
            ...
            'properties' => $properties, // 可加入筛选的属性
            'propertyFilters' => $propertyFilters, // 已加入筛选的属性
            'filterString' => $filterString, // 已加入筛选的属性的query格式
        ]);
    }
